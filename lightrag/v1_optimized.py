...
async def get_ranked_contexts(query: str, context_response: str, sem: asyncio.Semaphore):
    ...
    try:
        ...
        #     print(f"DEBUG: first char of Formatted contexts: {context_formatted[0][:10] if context_formatted[0] else 'empty'}")
        if len(context_formatted) > 7:
            async with sem:
                embeddings = await embed_client.embeddings.create(
                    input=[query] + context_formatted,
                    model="text-embedding-3-small"
                )
            print(f"DEBUG: embeddings: {embeddings[0]}")
            query_embedding = embeddings.data[0].embedding
            context_embeddings = [data.embedding for data in embeddings.data[1:]]
            scores = []
            ...
            print(f"DEBUG: contexts: {contexts[0]}")
            return contexts[:7]
        else:
            return contexts
    except Exception as e:
...
async def main() -> None:
    ...
    async def handle_one_qa(qa, sem, index):
        try:
            expanded_query = await expand_query(qa['question'], sem)
            response, contexts = await process_query(expanded_query, expanded_query, sem)
            qa['expanded_query'] = expanded_query
