...
async def fuse(context_list: list[str]):
    entities = []
    relationships = []
    docs = []
    entity_set = set()
    ...
    for context_response in context_list:
        ...
        for doc in docs_list:
            if doc['file_path'] not in doc_set:
                ...
                docs.append(doc)
    return entities, relationships, docs
...
async def main() -> None:
    ...
    async def handle_one_qa(qa, index):
        try:
            ...
            # chinese_chars = re.findall(r'[\u4e00-\u9fff]', step_back_query)
            # english_chars = re.findall(r'[A-Za-z]', step_back_query)
            # translated_query = step_back_query
            # if len(chinese_chars) > len(english_chars):
            #     translated_query = await translate_chinese_query(step_back_query)
            #     qa['translated_query'] = translated_query
            context_list = []
            ...
            entities, relationships, docs = await fuse(context_list)
            string = "-----Entities(KG)-----"
